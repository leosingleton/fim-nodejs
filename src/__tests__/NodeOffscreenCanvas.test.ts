...
describe('NodeOffscreenCanvas', () => {
  ...
  it('Creates and disposes', () => {
    const oc = NodeOffscreenCanvasFactory(100, 200, FimCanvasType.Canvas2D, null);
  ...
  it('Gets a WebGL rendering context', () => {
    const oc = NodeOffscreenCanvasFactory(100, 200, FimCanvasType.WebGL, null);
    const ctx = oc.getContext('webgl') as WebGLRenderingContext;
    expect(ctx).toBeDefined();
  });

  it('Exports to JPEG', async () => {
    const oc = NodeOffscreenCanvasFactory(100, 200, FimCanvasType.WebGL, null) as NodeOffscreenCanvas;
    const ctx = oc.getContext('webgl') as WebGLRenderingContext;
    expect(ctx).toBeDefined();

    const buffer = await oc.convertToBuffer({ type: 'image/jpeg', quality: 0.9 });
  ...
  it('Exports to PNG', async () => {
    ...
    const buffer = await oc.convertToBuffer({ type: 'image/png' });
