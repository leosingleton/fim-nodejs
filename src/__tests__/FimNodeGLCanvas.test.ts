...
describe('FimNodeGLCanvas', () => {
  ...
  it('Creates, fills, and disposes', () => {
    using(new FimNode() as Fim, fim => {
      const canvas = fim.createGLCanvas(100, 200, '#f00');
  ...
  it('Executes a WebGL fill shader', () => {
    using(new FimNode() as Fim, fim => {
      using(fim.createGLCanvas(100, 200, '#f00'), canvas => {
        const program = new FimGLProgramFill(canvas);
  ...
  it('Executes a WebGL copy shader', () => {
    using(new FimNode() as Fim, fim => {
      using(fim.createGLCanvas(100, 200, '#f00'), canvas => {
        const color = fim.createRgbaBuffer(100, 200, '#0f0');
        const texture = canvas.createTextureFrom(color);
        const program = new FimGLProgramCopy(canvas);
  ...
  it('Creates a texture from a FimNodeCanvas', () => {
    using(new FimNode() as Fim, fim => {
      using(fim.createGLCanvas(100, 200, '#f00'), canvas => {
        const color = fim.createCanvas(100, 200, '#0f0');
        const texture = canvas.createTextureFrom(color);
        const program = new FimGLProgramCopy(canvas);
        program.setInputs(texture);
        program.execute();
  ...
  it('Executes a Gaussian blur', () => {
    using(new FimNode() as Fim, fim => {
      using(fim.createGLCanvas(100, 200, '#f00'), canvas => {
        const color = fim.createRgbaBuffer(100, 200, '#00f');
        const texture = canvas.createTextureFrom(color);
        const program = new FimGLProgramMatrixOperation1D(canvas, 13);
  ...
  it('Executes a Gaussian blur with linear sampling', () => {
    using(new FimNode(), fim => {
      using(fim.createGLCanvas(100, 200, '#f00'), canvas => {
        const color = fim.createRgbaBuffer(100, 200, '#0f0');
        const texture = canvas.createTextureFrom(color, FimGLTextureFlags.LinearSampling);
        const program = new FimGLProgramMatrixOperation1DFast(canvas, 13);
        const kernel = GaussianKernel.calculate(2, 13);
        program.setInputs(texture, kernel);
        program.execute();
