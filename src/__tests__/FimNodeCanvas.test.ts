...
function expectToBeCloseTo(actual: FimColor, expectedColor: string): void {
  const expected = FimColor.fromString(expectedColor);
...
describe('FimNodeCanvas', () => {
  ...
  it('Creates, fills, and disposes', () => {
    using(new FimNode() as Fim, fim => {
      const canvas = fim.createCanvas(100, 200, '#f00');
  ...
  it('Imports from JPEG', async () => {
    const jpeg = new Uint8Array(Buffer.from(FimTestImages.fourSquaresJpegBase64, 'base64'));
  ...
  it('Exports to JPEG', async () => {
    await DisposableSet.usingAsync(async disposable => {
      const fim = new FimNode() as Fim;
      const canvas1 = disposable.addDisposable(fim.createCanvas(200, 100, '#f00'));
      const jpeg = await canvas1.toJpeg();
      ...
      // Decode the JPEG
      const canvas2 = disposable.addDisposable(await fim.createCanvasFromJpegAsync(jpeg));
  ...
  it('Copies from FimNodeCanvas', async () => {
    await DisposableSet.usingAsync(async disposable => {
      const fim = new FimNode() as Fim;
      const canvas1 = disposable.addDisposable(fim.createCanvas(240, 240, '#00f'));
      const canvas2 = disposable.addDisposable(fim.createCanvas(240, 240));
  ...
  it('Copies from FimNodeGLCanvas', async () => {
    await DisposableSet.usingAsync(async disposable => {
      ...
      const canvas = disposable.addDisposable(fim.createCanvas(240, 240));
      canvas.copyFrom(gl);
